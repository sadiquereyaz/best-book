...
class ViewModelAddress @Inject constructor(
    ...
    private val _state = MutableStateFlow(StateAddressScreen())
    val state = _state.asStateFlow()
//    private var randomTargetExam: String? = null

    init {
        onEvent(EventAddressScreen.Initialize)
    }

    fun onEvent(event: EventAddressScreen) {
        when (event) {
            EventAddressScreen.FetchBook -> fetchBooks()
            EventAddressScreen.FetchBanner -> fetchBanners()
            EventAddressScreen.Initialize -> {
